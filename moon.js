...
function setup()
{
    ...
    flags.visible = false;
    mwa = createSprite(rover.x,rover.y - 100,80,80);
    mwa.addImage(mwai);
    mwa.scale = 0.5;
    mwa.visible = false;
...
function draw()
{
    ...
    if (gameState === Play)
    {
        ...
        if (frameCount >=1 && frameCount <= 300)
        {
            ...
            stroke('black');
            fill('white');
            text("Hold Space key to decent Lander!",28,53);
        }
       ...
       if (player.scale <= 5 && player.scale > 0.199)
        ...
        if (player.scale <= 0.199 && (player.x < 560 || player.x > 595) && (player.y < 570 || player.y > 590))
        {
           player.addImage(lanb);
           gameState = end;
        }
        else if (player.scale <= 0.199 && (player.x > 560 || player.x < 595) && (player.y > 570 || player.y < 590))
        {
                if (keyDown('space'))
                {
                    rover.visible = true;
