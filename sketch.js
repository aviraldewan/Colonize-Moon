...
function setup() {
  ...
  createP("Rocket speed:");
  esc_vel = createSlider(5,20,10);
  ...
  rocket = createSprite(870,490,80,80);
...
function draw() {
  ...
  if (frameCount >= 0 && frameCount <= 200)
  {
    ...
    fill('yellow');
    text("Wait for the rocket to appear!",(displayWidth / 2) - 100,(displayHeight / 2 ) -  300);
  ...
  {  
    if (frameCount > 150 && frameCount < 400)
    {
      fill('yellow');
      textSize(20);
      text("TIP: Use slider above to adjust rocket's speed", rocket.x,rocket.y + 50);
    }
    b2.mousePressed(am);
    console.log(fuel);
    if (keyDown('up_arrow'))
   ...
   if (keyDown('left_arrow'))
    ...
    if (distance < 3080 && distance >= 1000)
    {
    ...
    else if (distance < 7000 && distance >= 5500)
    {
      ...
      textSize(20);
      fill('red');
      text("DEBRIES COLLISION ALERT", rocket.x - 150, rocket.y + 200);
      fill('yellow');
      textSize(20);
    ...
    else if (distance < 10000 && distance >= 7000)
    {
      ...
      textSize(20);
      fill('red');
      text("METEROITES COLLISION ALERT", rocket.x - 150, rocket.y + 200);
      spawn_checker();
    }
    //Put here
    if (distance <= 9500 && distance >= 9501)
    { 
